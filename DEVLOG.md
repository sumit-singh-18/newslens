...
## [2026-05-11] — Suggest an Outlet: already-in-list short-circuit + fresh-state reset

**`frontend/app.js`** only. Two fixes to **`SuggestOutletSection`**:

1. **Already in our verified list**: added **`findOutletInTiers(query)`** helper that case-insensitively matches the trimmed input (and any MBFC-canonical `outlet` name) against both **`name`** and **`domain`** fields of every entry in **`CREDIBLE_SOURCE_TIERS`** (the existing in-file mirror of **`backend/credible_domains.py`**), with a leading `www.` stripped. On hit, the component renders the green **`.suggest-outlet-success-card`** with copy "✓ [Outlet] is already in our verified sources list! You can find it in Tier [X] above." **instead of** the Domain / Why-add-this submission form — and suppresses the "Not found on Media Bias Fact Check" inline notice so the message stays singular.
2. **Clear state between searches**: **`handleCheck`** now resets **`domain`**, **`reason`**, and **`submitState`** (and records the current input as **`searchedName`** for the in-list comparison) before flipping **`lookup`** to **`"loading"`**, so clicking **Check Credibility** for a new outlet wipes any leftover submission inputs and previous result card.
...
## [2026-05-12] — Convert backend relative imports to absolute (Render `ImportError` fix)

Render boot was failing with **`ImportError: attempted relative import with no known parent package`** at **`backend/main.py:41`** (**`from .bias_utils import (...)`**) because the deployed process imports **`main`** as a top-level module from inside **`backend/`**, not as **`backend.main`**, so Python had no parent package for the leading-dot imports to resolve against.

Audited the backend with **`rg "^from \."`** and found relative imports in three modules: **`backend/main.py`** (six statements: **`bias_utils`**, **`credible_domains`**, **`database`**, **`framing_extract`**, **`news_fetcher`**, **`nlp_pipeline`**), **`backend/news_fetcher.py`** (four: **`credible_domains`**, **`database`**, **`framing_extract`**, **`nlp_pipeline`**), and **`backend/nlp_pipeline.py`** (two: **`bias_utils`**, **`database`**). All twelve statements were rewritten as plain absolute imports (**`from bias_utils import ...`** etc.). The other listed files (**`bias_utils.py`**, **`credible_domains.py`**, **`database.py`**, **`framing_extract.py`**) had no relative imports — already absolute. Files **`llm_analyzer.py`**, **`credibility_engine.py`**, and **`credibility_scores.py`** mentioned in the task don't exist in this repo and were skipped.

Updated **`backend/Procfile`** from **`cd .. && uvicorn backend.main:app ...`** to **`uvicorn main:app --host 0.0.0.0 --port $PORT`** so Render runs the app as a top-level module from inside **`backend/`**, matching the new flat-import layout.

Local verification (exactly as specified): **`cd backend && PYTHONPATH=. uvicorn main:app --port 8000`** — first attempt logged **`Application startup complete.`** with **no `ImportError`** (bind failed only because port 8000 was busy). Reran on port **`8765`** → **`Application startup complete.`** + **`Uvicorn running on http://127.0.0.1:8765`**, and **`curl /health`** returned **`HTTP 200`**. NLP pipeline loaded cleanly (**`Device set to use cpu`** ×2 for the sentiment + bias models).

Caveats: **`backend/tests/`** still imports via the package path (**`from backend.main import ...`**, etc.) and **`backend/__init__.py`** is unchanged. Tests run from the repo root via pytest will now fail because the inner modules expect each other on a flat **`sys.path`** rather than under the **`backend.`** namespace. Fix is to either (a) update test imports to **`from main import ...`** style and run pytest with **`PYTHONPATH=backend`**, or (b) add a **`conftest.py`** at **`backend/tests/`** that prepends **`backend/`** to **`sys.path`**. Not done in this commit — scope was the Render `ImportError`.

Pushed as **`fix: convert all relative imports to absolute`**.


## [2026-05-13] — HuggingFace Spaces Docker backend

Added **`backend/Dockerfile`** (**`python:3.11-slim`**, **`WORKDIR /app`**, **`ENV PORT=7860`**, **`EXPOSE 7860`**, **`uvicorn main:app`** on **`0.0.0.0:7860`**) plus **`backend/.dockerignore`** to shrink build context (**`__pycache__`**, **`.env`**, **`*.db`**, **`tests/`**, etc.). **`backend/main.py`**: reads **`port = int(os.getenv("PORT", 7860))`**, documents **`if __name__ == "__main__"`** entry with **`uvicorn.run(..., port=port)`**, and extends CORS with **`allow_origin_regex`** matching deployed Space origins (**`https://…\.hf\.space$`**). Verified **`requirements.txt`** against imports: pinned **`torch==2.8.0`** ( **`torch==2.9.0`** does not resolve on PyPI in this environment; Docker/Linux would fail the same way). There was no **`USE_ML_MODELS`** flag in the repo to remove — NLP always loads full HF models via **`NLPPipeline`**. Pushed as **`76f4c35`** on **`main`** (**`feat: add Dockerfile for HuggingFace Spaces`**).


## [2026-05-13] — HuggingFace Spaces Docker setup (revised)
